...
Créez un dépôt GitHub, par exemple `abdoulaye-kalidou.github.io` (remplacez par votre nom d'utilisateur) — ce nom exact rend le site accessible directement à la racine.
Mettez tous ces fichiers à la racine du dépôt.
Dans les paramètres du dépôt (`Settings` → `Pages`), choisissez la branche `main` et le dossier `/root`.
Votre site sera en ligne à `https://VOTRE-NOM.github.io/` après quelques minutes.
